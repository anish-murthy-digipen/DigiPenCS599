...
void recurseModelNodes(ModelData *meshdata, const aiScene *aiscene,
                       const aiNode *node, const aiMatrix4x4 &parentTr,
                       const int level = 0);
...
void VkApp::loadModel() {
  ...
  app->myCamera.reset(glm::vec3(6.026, 1.348, 7.284), 1.5, 55.99, -7.06, 0.57,
                      0.1, 1000.0);
  scLightAmb = vec3(0.2);
  ...
  app->myCamera.reset(glm::vec3(2.28, 1.68, 6.64), 0.7, -20.0, 10.66, 0.57, 0.1,
                      1000.0);
  scLightAmb = vec3(0.2);
  scLightInt = vec3(1.0f);
...
bool VkApp::loadModel(const std::string &filename, glm::mat4 transform) {
  ModelData meshdata;
  if (!meshdata.readAssimpFile(filename.c_str(), glm::mat4(1.0)))
    return false;
  ...
  // 5: Create an instance of Emitter with those values and push_back onto
  // lightList
  ...
  initBufferWrapFromData(object.matColorBuffer, cmdBuf, meshdata.materials,
                         flag);
  initBufferWrapFromData(object.matIndexBuffer, cmdBuf, meshdata.matIndx, flag);

  NAME(object.vertexBuffer.buffer, VK_OBJECT_TYPE_BUFFER,
       "object.vertexBuffer");
  NAME(object.vertexBuffer.buffer, VK_OBJECT_TYPE_BUFFER, "object.indexBuffer");
  NAME(object.vertexBuffer.buffer, VK_OBJECT_TYPE_BUFFER,
       "object.matColorBuffer");
  NAME(object.vertexBuffer.buffer, VK_OBJECT_TYPE_BUFFER,
       "object.matIndexBuffer");

  submitTempCmdBuffer(cmdBuf);

  // Creates all textures on the GPU
  auto txtOffset =
      static_cast<uint32_t>(m_objText.size()); // Offset is current size
  ...
  // Could provide multiple transform here to make a vector of instances of this
  // object.
  ObjInst instance;
  instance.transform = transform;
  instance.objIndex =
      static_cast<uint32_t>(m_objData.size()); // Index of current object
  ...
  desc.vertexAddress =
      getBufferDeviceAddress(m_device, object.vertexBuffer.buffer);
  desc.indexAddress =
      getBufferDeviceAddress(m_device, object.indexBuffer.buffer);
  desc.materialAddress =
      getBufferDeviceAddress(m_device, object.matColorBuffer.buffer);
  desc.materialIndexAddress =
      getBufferDeviceAddress(m_device, object.matIndexBuffer.buffer);
...
bool ModelData::readAssimpFile(const std::string &path, const mat4 &M) {
  ...
  aiMatrix4x4 modelTr(M[0][0], M[1][0], M[2][0], M[3][0], M[0][1], M[1][1],
  ...
  for (int i = 0; i < aiscene->mNumMaterials; i++) {
    ...
    if (!emit.IsBlack()) { // An emitter
      newmat.diffuse = {
          1, 1, 1}; // An emitter needs (1,1,1), else black screen!  WTF???
      ...
      if (AI_SUCCESS == hd)
        Kd = vec3(diff.r, diff.g, diff.b);
      if (AI_SUCCESS == hs)
        Ks = vec3(spec.r, spec.g, spec.b);
    ...
    if (AI_SUCCESS == mtl->GetTexture(aiTextureType_DIFFUSE, 0, &texPath)) {
      ...
      std::string xxx = fullPath.string();
      textures.push_back(xxx);
    }
...
void recurseModelNodes(ModelData *meshdata, const aiScene *aiscene,
  ...
  aiMatrix3x3 normalTr = aiMatrix3x3(
      childTr); // Really should be inverse-transpose for full generality
  ...
  for (unsigned int m = 0; m < node->mNumMeshes; ++m) {
    ...
    for (unsigned int t = 0; t < aimesh->mNumVertices; ++t) {
      ...
      aiVector3D ainrm = aimesh->HasNormals() ? normalTr * aimesh->mNormals[t]
                                              : aiVector3D(0, 0, 1);
      aiVector3D aitex = aimesh->HasTextureCoords(0)
  ...
  for (unsigned int i = 0; i < node->mNumChildren; ++i)
    recurseModelNodes(meshdata, aiscene, node->mChildren[i], childTr,
                      level + 1);
}
...
ImageWrap VkApp::readTextureFile(std::string fileName) {
  ...
  ImageWrap myImage;
  VkExtent2D texSize{static_cast<uint32_t>(texWidth),
  ...
  generateMipmap(myImage.image, VK_FORMAT_R8G8B8A8_UNORM, texWidth, texHeight,
                 mipLevels);

  return myImage;
}

void VkApp::generateMipmap(VkImage image, VkFormat imageFormat,
